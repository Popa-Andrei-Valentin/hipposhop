<template>
  ...
  <ul>
    ...
    <li
      class="category-item"
      :id="0"
    ><a @click="clickId(0); clickedFilter(0);treeOrigins('')">Toate
        produsele</a></li>

    <!-- Category Tree Loop -->
    <li
      v-for="categ in catTree"
      :key="categ.id"
    >
      <a
        class="category-item"
        :style="{ 'padding-left': categ.position + 'rem' }"
        @click="clickId(categ.id); clickedFilter(categ.id); treeOrigins(categ)"
        :id="categ.id"
      >
        {{ categ.name }}
      </a>
